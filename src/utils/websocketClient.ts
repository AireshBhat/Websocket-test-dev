 ...
 */
export async function createAuthenticatedClient(url: string, privateKeyHex: string) {
  // Parse private key
  ...
  const connectionPromise = new Promise<ReturnType<typeof createClientInterface>>((resolve, reject) => {
    // Message callback
    ...
    socket.onmessage = (event) => {
      try {
        ...
        if (message.type === "connection_established" && message.auth_required) {
 ...
 */
async function authenticate(socket: WebSocket, privateKeyBytes: Uint8Array, publicKeyHex: string) {
